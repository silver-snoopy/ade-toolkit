...
**Net competitive effect.** The report's "largest standards-rooting gap" (design-time security/privacy/NFR) is **closed**. *Primary-source re-verified 2026-06-26 (see note below):* on threat modeling ADE is **not first/only** — **gsd is a genuine design-time peer** (STRIDE + dispositions authored in PLAN.md behind a *hard* ship gate, not an optional side-gate as first drafted here). ADE's verified, narrower edge is the **integration mechanism**: mitigations become **author-separated tests written before code** (TDD) + ADRs + a routing signal, where gsd verifies mitigations by *retroactive grep against already-written code*. On **privacy-by-design** (data classification + PII flag) ADE **is** first/only — no surveyed system scaffolds it (gsd explicitly refers privacy/GDPR out rather than classifying; LINDDUN appears in zero repos), so privacy moves from 0/17 to design-time-covered-by-ADE. The report's "leapfrog thesis" is *half-executed*; the frontier moves on.

> **Re-verification note (2026-06-26).** Two primary-source agents re-read the relevant competitor repos. (1) *Security/privacy:* gsd-core is a real design-time-threat peer (`agents/gsd-planner.md`, `capabilities/security/capability.json` ship:pre gate); ECC/gstack are code-level audit tools; BMAD/case have no real threat step. No system does data-classification/LINDDUN privacy-by-design. (2) *Live verify:* only **workos/case** holds a true *hard* live gate (`agents/closer.md` blocks `gh pr create` without a `manual-tested` runtime marker); gstack `/qa` and oh-my-cc do real live verification but **advisory** (don't gate ship). (3) *Cross-model:* both gstack (Claude+Codex dual-voice, P1 gate on large diffs) and compound-engineering (separate-process Claude↔Codex pass) genuinely fan across model families — ADE is behind both.
...
*State:* ADE has **zero runtime evidence** — Phase 6 verifies test-coverage only (the G6 regression, a deliberate casualty of the G5 stack-agnosticism win). *Field (re-verified 2026-06-26):* only **`case`** holds a true *hard* live gate (`closer.md` blocks `gh pr create` without a `manual-tested` runtime-evidence marker when `src/` changed); `gstack` `/qa` and `oh-my-cc` `qa-tester` do real live-browser/tmux verification but **advisory** — they don't gate ship. So ADE is behind `case` on *hard-gating* and behind gstack/omcc on even *having* the capability — the bar is low and ADE clears neither. *Idea:* un-park the **functional/acceptance-test tier** spec (`docs/superpowers/specs/2026-06-22-live-verification-design.md`) — author-separated tests that boot the assembled artifact via stack-agnostic `run`/`serve`/`verify` slots in `.ade/ade-stack.md`, executed Phase 5, persisted as durable automation. Scope to where value is high (CLI/lib/service) and keep visual UI as `(manual)`. *Effort:* Medium. *Leverage:* **Highest** — it is the single most-cited ADE weakness and the one a skeptic notices first.
...
**C3 — Codex-tier parity.** Official Codex docs now describe explicit subagent workflows and project-scoped custom agents, so this is no longer an upstream-platform blocker. The risk has moved to ADE integration: prove generated `.codex/agents/*.toml` roles, phase prompts, hook wiring, and task transcripts actually preserve author-separation and blind verification instead of silently degrading to in-context convention. *Effort:* Low-Medium. *Leverage:* Medium.
